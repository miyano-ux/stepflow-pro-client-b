...
export const SUPPORTED_SOURCES = [
  ...
  },
  {
    ...
    color:         "#E11D48",
    bgColor:       "#FFF1F2",
    // ⚠️ sourceFields の key はズバットAPI（order/detail）のJSONキー。
    //    顧客側GAS _buildZubatCust / _handleZubatFudousan の extracted キーと一致させること。
    //    ・氏名(name)/カナ(name_kana) は "姓 名" 結合文字列 → GAS側で姓名分割
    //    ・property_address（住所）と exclude_billing_status/detail（課金除外）は
    //      _handleZubatFudousan 側で合成・平坦化して extracted に格納している
    //    ・sale_eagerness / sale_priority / sale_anxiety はアンケート項目。
    //      テスト案件では空だったため対応は推定（回答済み実案件で要確認）
    sourceFields: [
      { key: "id",                     label: "依頼者番号" },
      { key: "created_at",             label: "情報取得日" },
      { key: "send_count",             label: "送信社数" },
      { key: "property_type",          label: "物件種別" },
      { key: "property_address",       label: "住所（物件所在地）" },
      { key: "exclusive_area",         label: "専有面積／建物面積" },
      { key: "land_area",              label: "土地面積" },
      { key: "layout",                 label: "間取り" },
      { key: "building_ages",          label: "築年数" },
      { key: "property_status",        label: "物件の状態" },
      { key: "annual_rental_income",   label: "年間収入" },
      { key: "property_owner",         label: "物件の所有者" },
      { key: "assessment_reason",      label: "売却理由" },
      { key: "sale_time",              label: "売却時期" },
      { key: "assessment_method",      label: "査定方法" },
      { key: "visit_hope_date",        label: "訪問希望日" },
      { key: "contact_day",            label: "連絡希望日" },
      { key: "contact_time",           label: "希望連絡時間" },
      { key: "asking_price",           label: "希望売却価格" },
      { key: "demand",                 label: "ご要望" },
      { key: "name",                   label: "氏名" },
      { key: "name_kana",              label: "氏名（フリガナ）" },
      { key: "email",                  label: "メールアドレス" },
      { key: "tel",                    label: "電話番号" },
      { key: "current_address",        label: "現住所（依頼者の居住地）" },
      { key: "sale_eagerness",         label: "売却意向 ※要確認" },
      { key: "sale_priority",          label: "重視する項目 ※要確認" },
      { key: "sale_anxiety",           label: "不安に思うこと ※要確認" },
      { key: "age",                    label: "年齢" },
      { key: "exclude_billing_status", label: "課金除外：申請状況" },
      { key: "exclude_billing_detail", label: "課金除外：詳細" },
      { key: "memo",                   label: "メモ（対応履歴・要注意）" },
    ],
  },
];
