  ...
  return (
    <div
      ...
      style={{
        ...
        padding: compact ? "12px 14px" : "12px 20px",
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6,
        transition: "all 0.2s",
        transform: isOver ? "scale(1.03)" : "scale(1)",
        boxShadow: isOver ? `0 0 0 3px ${color}25` : "none",
        minHeight: compact ? 0 : 68,
      }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: 900, fontSize: compact ? 13 : 15, color }}>
        <span style={{ fontSize: compact ? 18 : 22 }}>{emoji}</span>
...
export default function KanbanBoard({
  ...
  // 右パネルを表示するか
  const hasRightPanel = rightTerminals.length > 0 || excludedStatuses.length > 0;
