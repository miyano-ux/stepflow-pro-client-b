        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>{filteredAndSorted.map(c => (
            <tr key={c.id}>
              ...
              <td style={s.tableTd}><div style={{ display: "flex", gap: "15px" }}><Link to={`/schedule/${c.id}`} style={{ textDecoration: "none", color: THEME.primary, fontWeight: "700" }}>状況</Link><Link to={`/edit/${c.id}`} style={{ textDecoration: "none", color: THEME.textMuted }}>編集</Link><button onClick={async () => { if(window.confirm("削除？")) { await api.post(GAS_URL, { action: "delete", id: c.id }); onRefresh(); } }} style={{ background: "none", border: "none", color: THEME.danger, cursor: "pointer" }}><Trash2 size={16}/></button></div></td>
            </tr>
          ))}</tbody>
        </table>
      </div>
...
function ColumnSettings({ displaySettings, formSettings, onRefresh }) {
  ...
  const handleSave = async () => {
    try { await api.post(GAS_URL, { action: "saveDisplaySettings", settings: items }); alert("保存完了"); onRefresh(); navigate("/"); } catch (e) { alert("失敗"); }
  };
  ...
  return (
    <Page title="項目の調整">
      <div style={{ maxWidth: "700px" }}>
        ...
        {items.map((it, i) => (
          <div key={it.name} draggable onDragStart={() => setDragIdx(i)} onDragOver={(e) => { e.preventDefault(); if (dragIdx===null||dragIdx===i) return; const n=[...items]; const d=n.splice(dragIdx,1)[0]; n.splice(i,0,d); setDragIdx(i); setItems(n); }} onDragEnd={() => setDragIdx(null)}
        ...
        <div style={{ display: "flex", gap: "15px", marginTop: "24px" }}><button onClick={handleSave} style={{ ...s.btn, ...s.btnPrimary, flex: 2 }}><Save size={20}/> 設定を保存</button><button onClick={() => navigate("/")} style={{ ...s.btn, ...s.btnSecondary, flex: 1 }}>戻る</button></div>
      </div>
    </Page>
  );
}
...
function CustomerForm({ formSettings, scenarios, onRefresh }) {
  ...
  useEffect(() => { 
    if(scenarios.length > 0) setScenarioID(scenarios[0].シナリオID); 
  }, [scenarios]);

  return (
    <Page title="新規顧客登録" topButton={<button onClick={() => navigate("/form-settings")} style={{ ...s.btn, ...s.btnSecondary }}>項目調整</button>}>
      <div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}>
        <form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "add", lastName, firstName, phone, data: formData, scenarioID }); onRefresh(); navigate("/"); }}>
          ...
          {formSettings.map(f => <div key={f.name} style={{marginBottom:"20px"}}><label style={{fontWeight:"700", display:"block", marginBottom:"8px"}}>{f.name}</label><DynamicField f={f} value={formData[f.name]} onChange={val => setFormData({...formData, [f.name]: val})} /></div>)}
...
function CustomerSchedule({ customers, deliveryLogs, onRefresh }) {
  ...
  {editLog && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}><div style={{ ...s.card, width: "500px", marginBottom: 0 }}><h3>配信内容の調整</h3><div style={{marginBottom:"15px"}}><label style={{fontSize:"12px"}}>配信日時</label><input type="datetime-local" style={s.input} value={editLog.t} onChange={e=>setEditLog({...editLog, t:e.target.value})} /></div><div style={{marginBottom:"20px"}}><label style={{fontSize:"12px"}}>内容</label><textarea style={{...s.input, height:"150px", resize:"none"}} value={editLog.m} onChange={e=>setEditLog({...editLog, m:e.target.value})} /></div><div style={{display:"flex", gap:"12px"}}><button onClick={async()=>{await api.post(GAS_URL,{action:"updateDeliveryTime",logId:editLog.ログID,newTime:editLog.t, newMessage:editLog.m}); onRefresh(); setEditLog(null);}} style={{...s.btn, ...s.btnPrimary, flex:1}}>保存</button><button onClick={()=>setEditLog(null)} style={{...s.btn, ...s.btnSecondary, flex:1}}>閉じる</button></div></div></div>)}</Page>);
}
