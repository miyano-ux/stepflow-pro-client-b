...
function DynamicField({ f, value, onChange }) {
  ...
  if (f.type === "date") return (
    <input type="date" style={s.input} required={f.required} value={value || ""} onChange={e => onChange(e.target.value)} />
  );
  return (
    <input style={s.input} type={f.type} required={f.required} value={value || ""} onChange={e => onChange(e.target.value)} placeholder={`${f.name}を入力`} />
  );
}
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (sortConfig.key) {
      res.sort((a, b) => {
        ...
        const isDate = formSettings.find(x => x.name === sortConfig.key)?.type === "date" || sortConfig.key === "登録日";
        if (isDate) {
...
function CustomerEdit({ customers = [], scenarios = [], formSettings = [], onRefresh }) {
  ...
  useEffect(() => { 
    if (c) { 
      setL(c["姓"] || ""); setF(c["名"] || ""); setP(c["電話番号"] || ""); 
      setFD(c); setS(c["シナリオID"]); 
    } 
  }, [c]);
      ...
      <div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}>
        <form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "update", id, lastName, firstName, phone, data: formData, status: c["配信ステータス"], scenarioID }); onRefresh(); nav("/"); }}>
          ...
          {formSettings.map(f => <div key={f.name} style={{marginBottom:"20px"}}><label style={{fontWeight:"700", display:"block", marginBottom:"8px"}}>{f.name}</label>
            <DynamicField f={f} value={formData[f.name]} onChange={v => setFD({...formData, [f.name]: v})} />
          </div>)}
          <div style={{ marginBottom: "32px", borderTop: `1px solid ${THEME.border}`, paddingTop: "24px" }}><label style={{fontWeight:"700", display:"block", marginBottom:"8px"}}>シナリオの変更</label>
            <select style={s.input} value={scenarioID} onChange={e => setS(e.target.value)}>{[...new Set(scenarios?.map(x => x.シナリオID))].map(id => <option key={id} value={id}>{id}</option>)}</select>
          </div>
          <button type="submit" style={{ ...s.btn, ...s.btnPrimary, width: "100%", padding: "14px" }}>保存</button>
        </form>
      </div>
    </Page>
  );
  ...
  return (
    <Page title="配信状況" subtitle={`${c["姓"]}${c["名"]} 様`}>
        ...
        {myLogs.length > 0 ? myLogs.map((log, i) => (
          <div key={i} style={{ ...s.card, borderLeft: `6px solid ${log["ステータス"] === "配信済み" ? THEME.success : THEME.primary}`, padding: "20px" }}>
            <div style={{display:"flex", justifyContent:"space-between"}}>
              ...
              {log["ステータス"] === "配信待ち" && <button onClick={()=>{ setEditLog({ ...log, t: new Date(new Date(log["配信予定日時"]).getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16), m: log["内容"] }); }} style={{color:THEME.primary, background:"none", border:"none", cursor:"pointer", fontWeight:"600"}}>編集</button>}
            </div>
            <div style={{marginTop:"15px", whiteSpace:"pre-wrap", fontSize:"14px"}}>{log["内容"]}</div>
          </div>
      ...
      {editLog && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}>
        <div style={{ ...s.card, width: "500px" }}><h3>配信の調整</h3>
          <input type="datetime-local" style={s.input} value={editLog.t} onChange={e=>setEditLog({...editLog, t:e.target.value})} />
          <textarea style={{...s.input, height:"150px", marginTop:"15px"}} value={editLog.m} onChange={e=>setEditLog({...editLog, m:e.target.value})} />
          <div style={{display:"flex", gap:"12px", marginTop:"20px"}}><button onClick={async()=>{await api.post(GAS_URL,{action:"updateDeliveryTime",logId:editLog["ログID"],newTime:editLog.t, newMessage:editLog.m}); onRefresh(); setEditLog(null);}} style={{...s.btn, ...s.btnPrimary, flex:1}}>保存</button><button onClick={()=>setEditLog(null)} style={{...s.btn, ...s.btnSecondary, flex:1}}>閉じる</button></div>
        </div>
      </div>)}
    </Page>
  );
...
// --- 残りのコンポーネント (ColumnSettings, UserManager, etc.) ---
