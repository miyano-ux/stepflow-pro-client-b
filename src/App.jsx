...
const s = {
  ...
  card: { backgroundColor: THEME.card, borderRadius: "16px", border: `1px solid ${THEME.border}`, padding: "32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" },
  input: { width: "100%", padding: "12px 16px", borderRadius: "10px", border: `1px solid ${THEME.border}`, fontSize: "14px", outline: "none", boxSizing: "border-box" },
...
function CustomerList({ customers, formSettings, onRefresh }) {
  const [visibleCols, setVisibleCols] = useState(() => JSON.parse(localStorage.getItem("sf_cols_v11") || '["姓", "名", "電話番号", "シナリオID"]'));
  const [tempCols, setTempCols] = useState(visibleCols);
  const [showColMenu, setShowColMenu] = useState(false);
  const [search, setSearch] = useState({});
  const [dragIdx, setDragIdx] = useState(null);

  useEffect(() => localStorage.setItem("sf_cols_v11", JSON.stringify(visibleCols)), [visibleCols]);
      ...
      {showColMenu && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.3)", zIndex: 2000, display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ ...s.card, width: "380px" }}>
            ...
            <p style={{ fontSize: "12px", color: THEME.textMuted, marginBottom: "20px" }}>ドラッグで並び替え、チェックで表示切替</p>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button onClick={() => { setVisibleCols(tempCols); setShowColMenu(false); }} style={{ ...s.btn, backgroundColor: THEME.primary, color: "white", flex: 1 }}>適用する</button>
...
function ScenarioForm({ scenarios, onRefresh }) {
  ...
  return (<Page title="シナリオ編集"><div style={{ ...s.card, maxWidth: "800px" }}><label>シナリオ名</label><input style={s.input} value={id} onChange={e=>setId(e.target.value)} disabled={!!editId} />
    ...
    <button onClick={async () => { if(!id) return alert("名必須"); setIsSaving(true); try { const cRes = await axios.get(`${GAS_URL}?mode=countAffected&scenarioID=${id}`); if (window.confirm(`${cRes.data.count}名の予約も一括更新されます。？`)) { await api.post(GAS_URL, { action: "saveScenario", scenarioID: id, steps }); alert("更新しました"); nav("/scenarios"); } } finally { setIsSaving(false); } }} disabled={isSaving} style={{ ...s.btn, backgroundColor: THEME.primary, color: "white", width: "100%" }}>保存して反映</button>
  </div></Page>);
}
