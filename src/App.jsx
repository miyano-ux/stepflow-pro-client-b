...
function ScenarioList({ scenarios = [], onRefresh }) {
  ...
  return (<Page title="シナリオ管理" topButton={<Link to="/scenarios/new" style={{...s.btn, ...s.btnPrimary, textDecoration:"none"}}><Plus size={18}/> 新規作成</Link>}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "24px" }}>
    {Object.entries(grouped).map(([id, steps]) => (<div key={id} style={s.card}><div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}><h3>{id}</h3><button onClick={async()=>{if(window.confirm("削除？")){await api.post(GAS_URL,{action:"deleteScenario",scenarioID:id});onRefresh();}}} style={{color:THEME.danger, background:"none", border:"none"}}><Trash2 size={18}/></button></div><p style={{color:THEME.textMuted, fontSize:"13px", margin:"10px 0"}}>{steps.length} ステップ</p><Link to={`/scenarios/edit/${encodeURIComponent(id)}`} style={{ ...s.btn, ...s.btnSecondary, width: "100%", textDecoration: "none" }}>構成編集</Link></div>))}</div></Page>);
}
...
function CustomerForm({ formSettings = [], scenarios = [], onRefresh }) {
  ...
  return (<Page title="新規登録" topButton={<button onClick={() => { const f = document.createElement("input"); f.type="file"; f.accept=".csv"; f.onchange=handleFile; f.click(); }} style={{ ...s.btn, ...s.btnPrimary }}><Upload size={18} /> CSV登録</button>}><div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); try { await api.post(GAS_URL, { action: "add", lastName:ln, firstName:fn, phone:ph, data:fd, scenarioID:sc }); onRefresh(); navigate("/"); } catch(err) { alert(err.message); } }}>
    <div style={{ display: "flex", gap: "20px", marginBottom: "20px" }}><div style={{ flex: 1 }}><label>姓 *</label><input style={s.input} required onChange={e => setLn(e.target.value)} /></div><div style={{ flex: 1 }}><label>名 *</label><input style={s.input} required onChange={e => setFn(e.target.value)} /></div></div>
    <label>電話番号 *</label><input style={{...s.input, marginBottom:"20px"}} required value={ph} onChange={e => setPh(e.target.value)} placeholder="09012345678" />{formSettings.map(f => <div key={f.name} style={{marginBottom:"20px"}}><label>{f.name}</label><DynamicField f={f} value={fd[f.name]} onChange={v => setFd({...fd, [f.name]: v})} /></div>)}
    <label>適用シナリオ</label><select style={{...s.input, marginBottom:"32px"}} value={sc} onChange={e => setSc(e.target.value)}>{[...new Set(scenarios?.map(x => x["シナリオID"]))].map(id => <option key={id} value={id}>{id}</option>)}</select><button type="submit" style={{ ...s.btn, ...s.btnPrimary, width: "100%", padding: "14px" }}>登録確定</button></form></div></Page>);
}
...
function CustomerEdit({ customers = [], scenarios = [], formSettings = [], onRefresh }) {
  ...
  return (<Page title="顧客編集"><div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "update", id, lastName:ln, firstName:fn, phone:ph, data: fd, status: c["配信ステータス"], scenarioID: sc }); onRefresh(); nav("/"); }}>
    ...
    <label style={{display:"block", marginTop:"20px"}}>シナリオ</label><select style={s.input} value={sc} onChange={e=>setSc(e.target.value)}>{[...new Set(scenarios?.map(x=>x["シナリオID"]))].map(id=><option key={id} value={id}>{id}</option>)}</select><button type="submit" style={{ ...s.btn, ...s.btnPrimary, width: "100%", marginTop: "32px"}}>保存</button></form></div></Page>);
}
