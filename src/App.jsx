/**
 * StepFlow Pro React Frontend (V12.1 Hardened Edition)
 * 準拠: 究極要件仕様書 V12 (Absolute Master Edition)
 * 修正内容: ホワイトアウト対策、変数名競合回避、項目調整機能の完全復元、日付レンジ検索の統合
 */
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
        <input type="date" style={{ ...styles.input, width: "140px" }} value={value.start || ""} onChange={e => onChange({ ...value, start: e.target.value })} />
        <span style={{ color: THEME.textMuted }}>〜</span>
        <input type="date" style={{ ...styles.input, width: "140px" }} value={value.end || ""} onChange={e => onChange({ ...value, end: e.target.value })} />
      </div>
    </div>
  );
}
...
function Sidebar({ onLogout }) {
  ...
  const m = [
    ...
    { n: "項目・表示設定", p: "/form-settings", i: <ListFilter size={18} /> },
...
function CustomerList({ customers = [], displaySettings = [], formSettings = [], onRefresh }) {
  ...
  const visibleCols = useMemo(() => displaySettings?.length > 0 ? displaySettings.filter(item => item.visible).map(item => item.name) : ["姓", "名", "電話番号", "シナリオID", "登録日"], [displaySettings]);
  const searchableCols = useMemo(() => displaySettings?.length > 0 ? displaySettings.filter(item => item.searchable).map(item => item.name) : ["姓", "電話番号", "登録日"], [displaySettings]);
  ...
  return (<Page title="顧客ダッシュボード" topButton={<div style={{ display: "flex", gap: "12px" }}><button onClick={() => downloadCSV([visibleCols, ...filtered.map(c => visibleCols.map(col => c[col]))], "export.csv")} style={{ ...styles.btn, ...styles.btnSecondary }}><Download size={18} /> CSV出力</button></div>}>
      ...
      {searchableCols.map(col => {
        ...
        return isDate ? (
          ...
          <div key={col} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <input placeholder={`${col}検索...`} style={{ ...styles.input, width: "160px" }} value={search[col] || ""} onChange={e => setSearch({...search, [col]: e.target.value})} />
          </div>
        );
      })}
      <button onClick={() => setSearch({})} style={{ ...styles.btn, background: "none", color: THEME.primary, fontWeight: "800" }}>リセット</button>
    </div>
    <div style={{ ...styles.card, padding: 0, overflow: "hidden" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{visibleCols.map(c => <th key={c} style={{ ...styles.tableTh, cursor: "pointer" }} onClick={() => setSort({ key: c, dir: (sort.key === c && sort.dir === 'asc') ? 'desc' : 'asc' })}>{c} {sort.key === c ? (sort.dir === 'asc' ? <ArrowUp size={12}/> : <ArrowDown size={12}/>) : <ArrowUpDown size={12} opacity={0.3}/>}</th>)}<th style={styles.tableTh}>操作</th></tr></thead>
      <tbody>{filtered.map(c => (<tr key={c.id} style={{ transition: "0.2s" }} onMouseEnter={e => e.currentTarget.style.backgroundColor = THEME.bg} onMouseLeave={e => e.currentTarget.style.backgroundColor = "transparent"}>
        {visibleCols.map(col => <td key={col} style={styles.tableTd}>{col === "シナリオID" ? <span style={styles.badge}>{c[col]}</span> : formatDate(c[col])}</td>)}
        <td style={styles.tableTd}><div style={{ display: "flex", gap: "12px" }}>
          <Link to={`/direct-sms/${c.id}`} style={{ ...styles.badge, textDecoration: "none", backgroundColor: THEME.primary, color: "white" }}><MessageSquare size={12}/> SMS</Link>
          ...
          <button onClick={async () => { if(window.confirm("削除しますか？")) { await apiCall.post(GAS_URL, { action: "delete", id: c.id }); onRefresh(); } }} style={{ background: "none", border: "none", color: THEME.danger, cursor: "pointer" }}><Trash2 size={16}/></button>
...
function CustomerForm({ formSettings = [], scenarios = [], onRefresh }) {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      try { await apiCall.post(GAS_URL, { action: "bulkAdd", customers }); alert("完了"); onRefresh(); navigate("/"); } catch (err) { alert(err.message); }
    }; reader.readAsText(file);
  };
  return (<Page title="新規登録" topButton={<button onClick={() => { const f = document.createElement("input"); f.type="file"; f.accept=".csv"; f.onchange=(e)=>handleFileUpload(e); f.click(); }} style={{ ...styles.btn, ...styles.btnPrimary }}><Upload size={18} /> CSV登録</button>}><div style={{ ...styles.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); try { await apiCall.post(GAS_URL, { action: "add", lastName, firstName, phone, data: formData, scenarioID }); onRefresh(); navigate("/"); } catch(err) { alert(err.message); } }}>
    <div style={{ display: "flex", gap: "20px", marginBottom: "20px" }}><div style={{ flex: 1 }}><label style={{fontWeight:"700"}}>姓 *</label><input style={styles.input} required onChange={e => setLastName(e.target.value)} /></div><div style={{ flex: 1 }}><label style={{fontWeight:"700"}}>名 *</label><input style={styles.input} required onChange={e => setFirstName(e.target.value)} /></div></div>
    <label style={{fontWeight:"700"}}>電話番号 *</label><input style={{...styles.input, marginBottom:"20px"}} required value={phone} onChange={e => setPhone(e.target.value)} placeholder="090..." />{formSettings.map(f => <div key={f.name} style={{marginBottom:"20px"}}><label style={{fontWeight:"700"}}>{f.name}</label><DynamicField f={f} value={formData[f.name]} onChange={v => setFormData({...formData, [f.name]: v})} /></div>)}
    <label style={{fontWeight:"700"}}>シナリオ</label><select style={{...styles.input, marginBottom:"32px"}} value={scenarioID} onChange={e => setScenarioID(e.target.value)}>{[...new Set(scenarios?.map(x => x["シナリオID"]))].map(id => <option key={id} value={id}>{id}</option>)}</select><button type="submit" style={{ ...styles.btn, ...styles.btnPrimary, width: "100%", padding: "14px" }}>確定</button></form></div></Page>);
}
...
function CustomerEdit({ customers = [], scenarios = [], formSettings = [], onRefresh }) {
  ...
  return (<Page title="編集"><div style={{ ...styles.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await apiCall.post(GAS_URL, { action: "update", id, lastName:ln, firstName:fn, phone:ph, data: fd, status: c["配信ステータス"], scenarioID: sc }); onRefresh(); nav("/"); }}>
    ...
    <label style={{fontWeight:"700"}}>電話番号</label><input style={styles.input} value={ph} onChange={e=>setPh(e.target.value)} />{formSettings.map(f => <div key={f.name} style={{marginTop:"20px"}}><label style={{fontWeight:"700"}}>{f.name}</label><DynamicField f={f} value={fd[f.name]} onChange={v=>setFd({...fd,[f.name]:v})} /></div>)}
    <label style={{display:"block", marginTop:"20px", fontWeight:"700"}}>シナリオ</label><select style={styles.input} value={sc} onChange={e=>setSc(e.target.value)}>{[...new Set(scenarios?.map(x=>x["シナリオID"]))].map(id=><option key={id} value={id}>{id}</option>)}</select><button type="submit" style={{ ...styles.btn, ...styles.btnPrimary, width: "100%", marginTop: "32px", padding: "14px" }}>保存</button></form></div></Page>);
}
...
function DirectSms({ customers = [], templates = [], onRefresh }) {
  ...
  return (<Page title="個別SMS" subtitle={`${c["姓"]} ${c["名"]} 様へ`}><Link to="/" style={{ display: "block", marginBottom: "24px", color: THEME.primary, textDecoration: "none", fontWeight: "700" }}>← 戻る</Link>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 350px", gap: "32px" }}><div><label style={{ fontWeight: "700", display: "block", marginBottom: "8px" }}>予定日時</label><SmartDateTimePicker value={time} onChange={setTime} /><textarea style={{ ...styles.input, height: "300px", resize: "none", marginTop: "24px" }} value={msg} onChange={e => setMsg(e.target.value)} placeholder="本文..." /><button onClick={async()=>{ await apiCall.post(GAS_URL,{action:"sendDirectSms",phone:c["電話番号"],customerName:`${c["姓"]} ${c["名"]}`,scheduledTime:time,message:msg}); alert("完了"); navigate("/"); }} style={{ ...styles.btn, ...styles.btnPrimary, width: "100%", marginTop: "24px" }}>確定</button></div>
        <div><h3 style={{ margin: "0 0 16px 0", fontSize: "16px" }}>テンプレート</h3><div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>{templates.map(t => (<div key={t.id} onClick={() => setMsg(replaceVariables(t.content, c))} style={{ ...styles.card, padding: "16px", cursor: "pointer", border: `1px solid ${THEME.border}`, transition:"0.2s" }} onMouseEnter={e=>e.currentTarget.style.borderColor=THEME.primary} onMouseLeave={e=>e.currentTarget.style.borderColor=THEME.border}><div style={{ fontWeight: "700", fontSize: "14px" }}>{t.name}</div><div style={{ fontSize: "12px", color: THEME.textMuted, marginTop:"4px" }}>{t.content.slice(0, 50)}...</div></div>))}</div></div></div></Page>);
}

// --- (7) テンプレート管理 ---
function TemplateManager({ templates = [], onRefresh }) {
  const [modal, setModal] = useState({ open: false, data: { id: "", name: "", content: "" } });
  return (<Page title="テンプレート管理" topButton={<button onClick={() => setModal({ open: true, data: { id: "", name: "", content: "{{姓}} {{名}} 様\n" } })} style={{ ...styles.btn, ...styles.btnPrimary }}><Plus size={18}/> 新規追加</button>}>
    ...
    {modal.open && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}><div style={{ ...styles.card, width: "600px" }}><h3>編集</h3><form onSubmit={async(e)=>{e.preventDefault(); await apiCall.post(GAS_URL,{action:"saveTemplate",...modal.data}); setModal({open:false}); onRefresh();}}><input style={{ ...styles.input, marginBottom: "16px" }} value={modal.data.name} onChange={e => setModal({...modal, data: {...modal.data, name: e.target.value}})} required /><textarea style={{ ...styles.input, height: "200px", resize: "none", marginBottom: "20px" }} value={modal.data.content} onChange={e => setModal({...modal, data: {...modal.data, content: e.target.value}})} required /><div style={{ display: "flex", gap: "12px" }}><button type="submit" style={{ ...styles.btn, ...styles.btnPrimary, flex: 1 }}>保存</button><button type="button" onClick={() => setModal({ open: false })} style={{ ...styles.btn, ...styles.btnSecondary, flex: 1 }}>閉じる</button></div></form></div></div>)}</Page>);
}

// --- (8) シナリオ一覧 ---
function ScenarioList({ scenarios = [], onRefresh }) {
  const grouped = (scenarios || []).reduce((acc, item) => { (acc[item["シナリオID"]] = acc[item["シナリオID"]] || []).push(item); return acc; }, {});
      ...
      <div key={id} style={{ ...styles.card, display: "flex", flexDirection: "column", padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "32px", flexGrow: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "20px" }}>
            ...
            <button onClick={async()=>{if(window.confirm("削除？")){await apiCall.post(GAS_URL,{action:"deleteScenario",scenarioID:id});onRefresh();}}} style={{ color: THEME.danger, background: "#FEF2F2", padding: "8px", borderRadius: "8px", border: "none", cursor: "pointer" }}><Trash2 size={18}/></button>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", color: THEME.textMuted, fontSize: "14px", fontWeight: "600", marginBottom: "24px" }}><Clock size={16} /> {steps.length} ステップ</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {steps.sort((a,b)=>a["ステップ数"]-b["ステップ数"]).slice(0,3).map((st, idx) => (
              <div key={idx} style={{ display: "flex", alignItems: "center", gap: "12px", fontSize: "13px", color: THEME.textMain }}>
        ...
        <div style={{ padding: "0 24px 24px 24px", boxSizing: "border-box" }}>
          <Link to={`/scenarios/edit/${encodeURIComponent(id)}`} style={{ ...styles.btn, ...styles.btnSecondary, width: "100%", textDecoration: "none", justifyContent: "space-between", padding: "14px 20px" }}>
            <span>構成を編集する</span><ChevronRight size={18} />
          </Link>
        </div>
      </div>
    ))}</div></Page>);
...
function ScenarioForm({ scenarios = [], onRefresh }) {
  ...
  const handleSave = async () => {
    if (!name.trim()) return alert("名前を入力！");
    setSaving(true);
    try {
      await apiCall.post(GAS_URL, { action: "saveScenario", scenarioID: name, steps: st });
      alert("完了"); onRefresh(); nav("/scenarios");
    } catch (e) { alert(e.message); } finally { setSaving(false); }
  };

  return (
    <Page title={id ? "編集" : "作成"} topButton={<button onClick={handleSave} disabled={saving} style={{...styles.btn, ...styles.btnPrimary}}>
      {saving ? <Loader2 size={18} className="animate-spin"/> : <Save size={18}/>} 保存</button>}
    >
      <div style={{ maxWidth: "850px", margin: "0 auto" }}>
        <div style={{ ...styles.card, marginBottom: "40px" }}>
          <label style={{ fontSize: "13px", fontWeight: "900", color: THEME.textMuted, display: "block", marginBottom: "12px" }}>シナリオ名</label>
...
function UserManager({ masterUrl }) {
  ...
  const sub = async (e) => { e.preventDefault(); await apiCall.post(masterUrl, { action: modal.mode === "add" ? "addUser" : "editUser", company: CLIENT_COMPANY_NAME, ...modal.data }); setModal({ open: false }); fetch(); };
  return (<Page title="ユーザー" topButton={<button onClick={() => setModal({ open: true, mode: "add", data: { name: "", email: "" } })} style={{ ...styles.btn, ...styles.btnPrimary }}><Plus size={18} /> 追加</button>}><div style={{ ...styles.card, padding: 0 }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th style={styles.tableTh}>名前</th><th style={styles.tableTh}>メール</th><th style={{...styles.tableTh, textAlign:"right"}}>操作</th></tr></thead><tbody>{users.map((u, i) => (<tr key={i}><td style={styles.tableTd}>{u.name}</td><td style={styles.tableTd}>{u.email}</td><td style={{...styles.tableTd, textAlign:"right"}}><div style={{display:"flex", gap:"10px", justifyContent:"flex-end"}}><button onClick={() => setModal({ open: true, mode: "edit", data: { name: u.name, email: u.email, oldEmail: u.email } })} style={{background:"none", border:"none", color:THEME.primary, cursor:"pointer"}}>編集</button><button onClick={async()=>{if(window.confirm("削除？")){await apiCall.post(masterUrl,{action:"deleteUser",company:CLIENT_COMPANY_NAME,email:u.email});fetch();}}} style={{background:"none", border:"none", color:THEME.danger, cursor:"pointer"}}><Trash2 size={16}/></button></div></td></tr>))}</tbody></table></div>{modal.open && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}><div style={{ ...styles.card, width: "400px" }}><h3>編集</h3><form onSubmit={sub}><input style={{...styles.input, marginBottom:"15px"}} value={modal.data.name} onChange={e=>setModal({...modal, data:{...modal.data, name: e.target.value}})} placeholder="名前" required /><input style={{...styles.input, marginBottom:"15px"}} type="email" value={modal.data.email} onChange={e=>setModal({...modal, data:{...modal.data, email: e.target.value}})} placeholder="メール" required /><div style={{ display: "flex", gap: "10px" }}><button type="submit" style={{ ...styles.btn, ...styles.btnPrimary, flex: 1 }}>保存</button><button type="button" onClick={() => setModal({ open: false })} style={{ ...styles.btn, ...styles.btnSecondary, flex: 1 }}>閉じる</button></div></form></div></div>)}</Page>);
}
