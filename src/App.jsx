...
function Sidebar({ onLogout, user }) {
  const l = useLocation();
  const m = [{ n: "ダッシュボード", p: "/", i: <LayoutDashboard size={20} /> }, { n: "新規登録", p: "/add", i: <UserPlus size={20} /> }, { n: "シナリオ管理", p: "/scenarios", i: <Settings size={20} /> }, { n: "ユーザー管理", p: "/users", i: <Users size={20} /> }];
  return (
    <div style={s.sidebar}>
...
function CustomerList({ customers, formSettings, onRefresh }) {
  const [visibleCols, setVisibleCols] = useState(() => JSON.parse(localStorage.getItem("sf_cols_v5") || '["姓", "名", "電話番号"]'));
  const [showColMenu, setShowColMenu] = useState(false);
  const [search, setSearch] = useState({});

  useEffect(() => localStorage.setItem("sf_cols_v5", JSON.stringify(visibleCols)), [visibleCols]);
  ...
  return (
    <Page title="ダッシュボード" topButton={<button onClick={() => setShowColMenu(!showColMenu)} style={{ ...s.btn, background: "white", color: THEME.textMain, border: `1px solid ${THEME.border}` }}><ListFilter size={18} /> 表示項目</button>}>
      ...
      <div style={{ ...s.card, padding: 0 }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{visibleCols.map(c => <th key={c} style={s.tableTh}>{c}</th>)}<th style={s.tableTh}>操作</th></tr></thead><tbody>{filtered.map(c => (<tr key={c.id}>{visibleCols.map(col => <td key={col} style={s.tableTd}>{c[col] || "-"}</td>)}<td style={s.tableTd}><div style={{ display: "flex", gap: "12px" }}><Link to={`/schedule/${c.id}`} style={{ textDecoration: "none", color: THEME.primary, fontWeight: "700" }}>状況</Link><Link to={`/edit/${c.id}`} style={{ textDecoration: "none", color: THEME.textMuted }}>編集</Link><button onClick={async () => { if(window.confirm("削除？")) { await api.post(GAS_URL, { action: "delete", id: c.id }); onRefresh(); } }} style={{ background: "none", border: "none", color: THEME.danger }}><Trash2 size={16}/></button></div></td></tr>))}</tbody></table></div>
    </Page>
  );
}
...
function CustomerForm({ formSettings, scenarios, onRefresh }) {
  ...
  return (
    <Page title="新規顧客登録" topButton={<button onClick={() => navigate("/form-settings")} style={{ ...s.btn, background: THEME.bg, color: THEME.primary, border: `1px solid ${THEME.border}` }}><ListFilter size={18} /> 項目を調整</button>}>
      <div style={{ ...s.card, maxWidth: "650px" }}><form onSubmit={async (e) => { e.preventDefault(); if (!validateTel(phone)) return alert("番号不正"); await api.post(GAS_URL, { action: "add", lastName, firstName, phone, data: formData, scenarioID }); onRefresh(); navigate("/"); }}>
...
function CustomerEdit({ customers, scenarios, formSettings, onRefresh }) {
  ...
  useEffect(() => { if (c) { setLastName(c["姓"] || ""); setFirstName(c["名"] || ""); setPhone(c["電話番号"] || ""); setFormData(c); setScenarioID(c.シナリオID); } }, [c]);
  ...
  return (
    <Page title="情報の編集">
      <div style={{ ...s.card, maxWidth: "650px" }}><form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "update", id, lastName, firstName, phone, data: formData, status: c.配信ステータス, scenarioID }); onRefresh(); nav("/"); }}>
        ...
        <div style={{ borderTop: `1px solid ${THEME.border}`, paddingTop: "20px", marginTop: "10px" }}>{formSettings.map(f => <div key={f.name}><label>{f.name}</label><DynamicField f={f} value={formData[f.name]} onChange={val => setFormData({ ...formData, [f.name]: val })} /></div>)}</div>
        <label>適用シナリオ</label><select style={s.input} value={scenarioID} onChange={e => setScenarioID(e.target.value)}>{[...new Set(scenarios.map(x => x.シナリオID))].map(id => <option key={id} value={id}>{id}</option>)}</select>
        <button type="submit" style={{ ...s.btn, width: "100%", padding: "16px" }}>保存</button>
      </form></div>
    </Page>
  );
}
...
function ScenarioForm({ scenarios, onRefresh }) {
  ...
  return (
    <Page title="シナリオの構成">
      <div style={s.card}><label>シナリオ名</label><input style={s.input} value={id} onChange={e=>setId(e.target.value)} disabled={!!editId} />
        ...
        <button onClick={async () => { if(!id) return alert("名前必須"); setIsSaving(true); const countRes = await axios.get(`${GAS_URL}?mode=countAffected&scenarioID=${id}`); if (window.confirm(`${countRes.data.count}名の予約も更新されます。？`)) { await api.post(GAS_URL, { action: "saveScenario", scenarioID: id, steps }); onRefresh(); navigate("/scenarios"); } setIsSaving(false); }} disabled={isSaving} style={{ ...s.btn, width: "100%" }}>保存して反映</button>
      </div>
    </Page>
  );
}

function CustomerSchedule({ customers, deliveryLogs }) {
  ...
  return (<Page title="状況" subtitle={`${c["姓"]}${c["名"]} 様`}><Link to="/" style={{display:"block", marginBottom:"24px"}}>← 戻る</Link><div style={{display:"flex", flexDirection:"column", gap:"16px"}}>{myLogs.map((log, i) => (<div key={i} style={{ ...s.card, borderLeft: `6px solid ${THEME.primary}` }}><div style={{fontWeight:"800"}}>{new Date(log.配信予定日時).toLocaleString('ja-JP')} 【{log.ステータス}】</div><div style={{marginTop:"8px", fontSize:"14px", whiteSpace:"pre-wrap"}}>{log.内容}</div></div>))}</div></Page>);
}
