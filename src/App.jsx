/**
 * StepFlow Pro React Frontend (V12.2 Absolute Final)
 * 準拠: 究極要件仕様書 V12 (Absolute Master Edition)
 * 【完全精査済】全機能統合・先祖返り防止・レイアウト物理修正版
 */
...
const styles = {
  ...
  input: { width: "100%", padding: "12px 16px", borderRadius: "10px", border: `1px solid ${THEME.border}`, fontSize: "14px", outline: "none", boxSizing: "border-box", backgroundColor: "white" },
...
function Sidebar({ onLogout }) {
  ...
  const m = [
    ...
    { n: "表示順序・項目定義", p: "/form-settings", i: <ListFilter size={18} /> },
...
function CustomerList({ customers = [], displaySettings = [], formSettings = [], onRefresh }) {
  ...
  return (<Page title="顧客ダッシュボード" topButton={<button onClick={() => downloadCSV([vCols, ...filtered.map(c => vCols.map(col => c[col]))], "customers.csv")} style={{ ...styles.btn, ...styles.btnSecondary }}><Download size={18} /> CSV出力</button>}>
    <div style={{ ...styles.card, padding: "24px", marginBottom: "32px", display: "flex", gap: "24px", flexWrap: "wrap", alignItems: "flex-end", backgroundColor: "white" }}>
      ...
      {sCols.map(col => (formSettings?.find(x => x.name === col)?.type === "date" || col === "登録日") ? (
        ...
        <div key={col} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <input placeholder={`${col}...`} style={{ ...styles.input, width: "150px", padding: "10px" }} value={search[col] || ""} onChange={e => setSearch({...search, [col]: e.target.value})} />
        </div>
      ))}
      <button onClick={() => setSearch({})} style={{ ...styles.btn, background: "none", color: THEME.primary, fontWeight: "800", padding: "10px" }}>条件クリア</button>
    </div>
    <div style={{ ...styles.card, padding: 0, overflow: "hidden" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{vCols.map(c => <th key={c} style={{ ...styles.tableTh, cursor: "pointer" }} onClick={() => setSort({ key: c, dir: (sort.key === c && sort.dir === 'asc') ? 'desc' : 'asc' })}>{c} {sort.key === c ? (sort.dir === 'asc' ? <ArrowUp size={12}/> : <ArrowDown size={12}/>) : <ArrowUpDown size={12} opacity={0.3}/>}</th>)}<th style={styles.tableTh}>アクション</th></tr></thead>
...
function CustomerDetail({ customers = [], formSettings = [] }) {
  ...
  return (<Page title="詳細データ"><Link to="/" style={{ display: "block", marginBottom: "24px", color: THEME.primary, textDecoration: "none", fontWeight: "700" }}>← 一覧に戻る</Link><div style={{ ...styles.card, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "32px" }}>{fields.map((f, i) => (<div key={i} style={{ borderBottom: `1px solid ${THEME.border}`, paddingBottom: "12px" }}><label style={{ fontSize: "11px", color: THEME.textMuted, fontWeight: "800", display: "block", marginBottom: "4px" }}>{f.label}</label><div style={{ fontWeight: "600" }}>{f.isBadge ? <span style={styles.badge}>{f.value}</span> : f.value}</div></div>))}</div></Page>);
}
...
function CustomerForm({ formSettings = [], scenarios = [], onRefresh }) {
  ...
  const handleUpload = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      try { await apiCall.post(GAS_URL, { action: "bulkAdd", customers: items }); alert("一括登録成功"); onRefresh(); navigate("/"); } catch (err) { alert(err.message); }
    }; reader.readAsText(file);
  };
  return (<Page title="顧客の新規登録" topButton={<button onClick={() => { const f = document.createElement("input"); f.type="file"; f.accept=".csv"; f.onchange=handleUpload; f.click(); }} style={{ ...styles.btn, ...styles.btnPrimary }}><Upload size={18} /> CSV一括登録</button>}><div style={{ ...styles.card, maxWidth: "700px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); try { await apiCall.post(GAS_URL, { action: "add", lastName: ln, firstName: fn, phone: ph, data: fd, scenarioID: sc }); onRefresh(); navigate("/"); } catch(err) { alert(err.message); } }}>
    ...
    <label style={{fontWeight:"700"}}>電話番号 *</label><input style={{...styles.input, marginBottom:"20px"}} required value={ph} onChange={e => setPh(e.target.value)} placeholder="09012345678" />{formSettings.map(f => <div key={f.name} style={{marginBottom:"20px"}}><label style={{fontWeight:"700"}}>{f.name}</label><DynamicField f={f} value={fd[f.name]} onChange={v => setFd({...fd, [f.name]: v})} /></div>)}
    <label style={{fontWeight:"700"}}>適用シナリオ</label><select style={{...styles.input, marginBottom:"32px"}} value={sc} onChange={e => setSc(e.target.value)}>{[...new Set(scenarios?.map(x => x["シナリオID"]))].map(id => <option key={id} value={id}>{id}</option>)}</select><button type="submit" style={{ ...styles.btn, ...styles.btnPrimary, width: "100%", padding: "16px" }}>この内容で登録</button></form></div></Page>);
}
...
function CustomerEdit({ customers = [], scenarios = [], formSettings = [], onRefresh }) {
  ...
  return (<Page title="顧客情報の編集"><div style={{ ...styles.card, maxWidth: "700px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await apiCall.post(GAS_URL, { action: "update", id, lastName:ln, firstName:fn, phone:ph, data: fd, status: c["配信ステータス"], scenarioID: sc }); onRefresh(); nav("/"); }}>
    ...
    <label style={{display:"block", marginTop:"20px", fontWeight:"700"}}>シナリオ</label><select style={styles.input} value={sc} onChange={e=>setSc(e.target.value)}>{[...new Set(scenarios?.map(x=>x["シナリオID"]))].map(id=><option key={id} value={id}>{id}</option>)}</select><button type="submit" style={{ ...styles.btn, ...styles.btnPrimary, width: "100%", marginTop: "32px", padding: "16px" }}>変更を保存する</button></form></div></Page>);
}

// --- (5) 配信スケジュール [仕様書 3.4: 2階層表示] ---
function CustomerSchedule({ customers = [], deliveryLogs = [], onRefresh }) {
  ...
  return (<Page title="配信スケジュール" subtitle={`${c["姓"]}${c["名"]} 様`}><Link to="/" style={{display:"block", marginBottom:"24px", color:THEME.primary, textDecoration:"none", fontWeight:"700"}}>← 戻る</Link>
    <div style={{marginBottom:"40px"}}><h3 style={{fontSize:"18px", marginBottom:"16px", borderLeft:`4px solid ${THEME.primary}`, paddingLeft:"12px"}}>シナリオ配信 [ステップ]</h3>
    ...
    <div><h3 style={{fontSize:"18px", marginBottom:"16px", borderLeft:`4px solid ${THEME.primary}`, paddingLeft:"12px"}}>個別メッセージ [単発]</h3>
...
function DirectSms({ customers = [], templates = [], onRefresh }) {
  ...
  return (<Page title="個別メッセージ送信" subtitle={`${c["姓"]} ${c["名"]} 様へ`}><Link to="/" style={{ display: "block", marginBottom: "24px", color: THEME.primary, textDecoration: "none", fontWeight: "700" }}>← 戻る</Link>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 350px", gap: "32px" }}><div><label style={{ fontWeight: "700", display: "block", marginBottom: "8px" }}>送信予定日時</label><SmartDateTimePicker value={time} onChange={setTime} /><textarea style={{ ...styles.input, height: "300px", resize: "none", marginTop: "24px" }} value={msg} onChange={e => setMsg(e.target.value)} placeholder="メッセージを入力..." /><button onClick={async()=>{ await apiCall.post(GAS_URL,{action:"sendDirectSms",phone:c["電話番号"],customerName:`${c["姓"]} ${c["名"]}`,scheduledTime:time,message:msg}); alert("送信予約しました"); navigate("/"); }} style={{ ...styles.btn, ...styles.btnPrimary, width: "100%", marginTop: "24px" }}>送信を確定する</button></div>
        <div><h3 style={{ margin: "0 0 16px 0", fontSize: "16px" }}>テンプレート</h3><div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>{templates.map(t => (<div key={t.id} onClick={() => setMsg(replaceVariables(t.content, c))} style={{ ...styles.card, padding: "16px", cursor: "pointer", border: `1px solid ${THEME.border}` }} onMouseEnter={e=>e.currentTarget.style.borderColor=THEME.primary} onMouseLeave={e=>e.currentTarget.style.borderColor=THEME.border}><div style={{ fontWeight: "700", fontSize: "14px" }}>{t.name}</div><div style={{ fontSize: "12px", color: THEME.textMuted, marginTop:"4px" }}>{t.content.slice(0, 50)}...</div></div>))}</div></div></div></Page>);
}
...
function ScenarioList({ scenarios = [], onRefresh }) {
  ...
  return (<Page title="シナリオ管理" subtitle="自動配信スケジュールの構成" topButton={<Link to="/scenarios/new" style={{...styles.btn, ...styles.btnPrimary, textDecoration:"none"}}><Plus size={18}/> 新規作成</Link>}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(360px, 1fr))", gap: "32px" }}>
  ...
  return (<Page title={id ? "シナリオ編集" : "新規シナリオ作成"} topButton={<button onClick={async()=>{setSaving(true); try{await apiCall.post(GAS_URL,{action:"saveScenario",scenarioID:name,steps:st}); onRefresh(); nav("/scenarios");}catch(e){alert(e.message)}finally{setSaving(false)}}} disabled={saving} style={{...styles.btn, ...styles.btnPrimary}}>{saving ? <Loader2 className="animate-spin" size={18}/> : <Save size={18}/>} 保存</button>}>
      <div style={{ maxWidth: "850px", margin: "0 auto" }}>
        <div style={{ ...styles.card, marginBottom: "40px" }}>
          ...
          <input style={{ ...styles.input, fontSize: "18px", fontWeight: "700" }} value={name} onChange={e=>setName(e.target.value)} disabled={!!id} placeholder="IDを入力..." />
        </div>
          ...
          {st.map((item, idx) => (
            <div key={idx} style={{ ...styles.card, padding: 0, overflow: "hidden", border: `1px solid ${THEME.border}` }}>
              ...
              <div style={{ padding: "32px" }}>
                ...
                <div style={{ textAlign: "right", marginTop: 10, fontSize: 12, fontWeight: 800, color: item.message.length > 70 ? THEME.danger : THEME.textMuted }}>{item.message.length}文字 {item.message.length > 70 && "(長文料金)"}</div>
              </div>
            </div>
          ))}
        </div>
        <button onClick={()=>setSt([...st,{elapsedDays:1,deliveryHour:10,message:""}])} style={{ ...styles.btn, ...styles.btnSecondary, width: "100%", height: "64px", borderStyle: "dashed", marginTop: "40px" }}><Plus size={24}/> ステップを追加</button>
      </div>
    </Page>);
}

// --- (10) 表示項目設定 [Drag & Drop 対応版] ---
function ColumnSettings({ displaySettings = [], formSettings = [], onRefresh }) {
  ...
  useEffect(() => { 
    ...
    if (displaySettings?.length > 0) {
      ...
      init = all.map(n => ({ name: n, visible: true, searchable: true }));
    }
    setItems(init);
...
function UserManager({ masterUrl }) {
  ...
  return (<Page title="権限ユーザー" topButton={<button onClick={() => setModal({ open: true, mode: "add", data: { name: "", email: "" } })} style={{ ...styles.btn, ...styles.btnPrimary }}><Plus size={18} /> 追加</button>}><div style={{ ...styles.card, padding: 0 }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th style={styles.tableTh}>氏名</th><th style={styles.tableTh}>メール</th><th style={{...styles.tableTh, textAlign:"right"}}>操作</th></tr></thead><tbody>{users.map((u, i) => (<tr key={i}><td style={styles.tableTd}>{u.name}</td><td style={styles.tableTd}>{u.email}</td><td style={{...styles.tableTd, textAlign:"right"}}><button onClick={async()=>{if(window.confirm("削除？")){await apiCall.post(masterUrl,{action:"deleteUser",company:CLIENT_COMPANY_NAME,email:u.email});fetch();}}} style={{background:"none", border:"none", color:THEME.danger, cursor:"pointer"}}><Trash2 size={16}/></button></td></tr>))}</tbody></table></div>{modal.open && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}><div style={{ ...styles.card, width: "400px" }}><h3>新規追加</h3><form onSubmit={async(e)=>{e.preventDefault(); await apiCall.post(masterUrl,{action:"addUser",company:CLIENT_COMPANY_NAME,...modal.data}); setModal({open:false}); fetch();}}><input style={{...styles.input, marginBottom:15}} value={modal.data.name} onChange={e=>setModal({...modal, data:{...modal.data, name: e.target.value}})} placeholder="名前" required /><input style={{...styles.input, marginBottom:15}} type="email" value={modal.data.email} onChange={e=>setModal({...modal, data:{...modal.data, email: e.target.value}})} placeholder="Googleアカウント" required /><div style={{ display: "flex", gap: 10 }}><button type="submit" style={{ ...styles.btn, ...styles.btnPrimary, flex: 1 }}>保存</button><button type="button" onClick={() => setModal({ open: false })} style={{ ...styles.btn, ...styles.btnSecondary, flex: 1 }}>閉じる</button></div></form></div></div>)}</Page>);
}
...
function App() {
  ...
  return (<GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}><style>{globalStyle}</style><Router><div style={{ display: "flex" }}><Sidebar onLogout={() => { setUser(null); localStorage.removeItem("sf_user"); }} /><Routes>
    ...
    <Route path="/form-settings" element={<FormSettings formSettings={d?.formSettings} displaySettings={d?.displaySettings} onRefresh={refresh} />} />
