...
import { 
  ...
  Plus, Loader2, LogOut, Users, GripVertical, ListFilter, Edit3, Lock, Save, Search, Clock, ArrowUpDown, ArrowUp, ArrowDown, Download, Upload, FileSpreadsheet, Info
} from "lucide-react";
    ...
    let res = [...customers].filter(c => {
      return Object.keys(search).every(key => {
        ...
        if (f?.type === "date" || key === "登録日") {
          ...
          const start = query.start ? new Date(query.start).setHours(0,0,0,0) : null;
          const end = query.end ? new Date(query.end).setHours(23,59,59,999) : null;
...
function ScenarioList({ scenarios = [], onRefresh }) {
  ...
  return (
    <Page title="シナリオ管理" topButton={<Link to="/scenarios/new" style={{...s.btn, ...s.btnPrimary, textDecoration:"none"}}><Plus size={18}/> 新規作成</Link>}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "24px" }}>
        {Object.entries(grouped).map(([id, steps]) => (
          <div key={id} style={s.card}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"15px"}}>
              <div><h3 style={{margin:0}}>{id}</h3><p style={{fontSize:"12px", color:THEME.textMuted}}>{steps.length} ステップ</p></div>
              <button onClick={async()=>{if(window.confirm("削除？")){await api.post(GAS_URL,{action:"deleteScenario",scenarioID:id});onRefresh();}}} style={{color:THEME.danger, background:"none", border:"none"}}><Trash2 size={18}/></button>
            </div>
            <Link to={`/scenarios/edit/${encodeURIComponent(id)}`} style={{ ...s.btn, ...s.btnSecondary, width: "100%", textDecoration: "none" }}>構成を編集</Link>
          </div>
        ))}
      </div>
    </Page>
...
function ScenarioForm({ scenarios = [], onRefresh }) {
  ...
  const [steps, setSteps] = useState([{ elapsedDays: 1, deliveryHour: 10, message: "" }]);

  useEffect(() => {
    if (editId) {
      const decodedId = decodeURIComponent(editId);
      setName(decodedId);
      const existing = scenarios.filter(s => s["シナリオID"] === decodedId).sort((a,b) => a["ステップ数"] - b["ステップ数"]);
      if (existing.length) setSteps(existing.map(s => ({ elapsedDays: s["経過日数"], deliveryHour: s["配信時間"], message: s["message"] })));
    }
  }, [editId, scenarios]);

  const handleSave = async () => {
    if(!name) return alert("名前は必須です");
    await api.post(GAS_URL, { action: "saveScenario", scenarioID: name, steps });
    alert("保存完了"); onRefresh(); navigate("/scenarios");
  };

  return (
    <Page title={editId ? "シナリオの編集" : "新規シナリオ作成"}>
      <div style={{ ...s.card, maxWidth: "800px" }}>
        <div style={{marginBottom:"30px"}}><label style={{fontWeight:"700", display:"block", marginBottom:"8px"}}>シナリオ名</label><input style={s.input} value={name} onChange={e=>setName(e.target.value)} disabled={!!editId} /></div>
        {steps.map((step, i) => (
          <div key={i} style={{ padding: "20px", background: "#F8FAFC", marginBottom: "20px", borderRadius: "12px", border: `1px solid ${THEME.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "15px" }}><span style={{ fontWeight: "900" }}>STEP {i + 1}</span><button onClick={() => setSteps(steps.filter((_, idx) => idx !== i))} style={{color:THEME.danger, background:"none", border:"none"}}><Trash2 size={16}/></button></div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "15px" }}>
              <div><label style={{fontSize:"12px"}}>登録からの経過日数</label><input type="number" style={s.input} value={step.elapsedDays} onChange={e=>{const n=[...steps];n[i].elapsedDays=e.target.value;setSteps(n)}} /></div>
              <div><label style={{fontSize:"12px"}}>配信時間(時)</label><input type="number" style={s.input} value={step.deliveryHour} onChange={e=>{const n=[...steps];n[i].deliveryHour=e.target.value;setSteps(n)}} /></div>
            </div>
            <label style={{fontSize:"12px"}}>メッセージ本文</label><textarea style={{ ...s.input, height: "100px", resize:"none" }} value={step.message} onChange={e=>{const n=[...steps];n[i].message=e.target.value;setSteps(n)}} />
          </div>
        ))}
        <button onClick={() => setSteps([...steps, { elapsedDays: 1, deliveryHour: 10, message: "" }])} style={{ ...s.btn, ...s.btnSecondary, width: "100%", marginBottom: "20px", borderStyle: "dashed" }}>+ ステップを追加</button>
        <button onClick={handleSave} style={{ ...s.btn, ...s.btnPrimary, width: "100%", padding: "14px" }}><Save size={18}/> シナリオを保存</button>
      </div>
    </Page>
  );
}
...
function CustomerDetail({ customers = [], formSettings = [] }) {
  ...
  return (<Page title="顧客詳細"><Link to="/" style={{display:"block", marginBottom:"24px", color:THEME.primary, textDecoration:"none", fontWeight:"700"}}>← 戻る</Link>
    <div style={{...s.card, display:"grid", gridTemplateColumns:"1fr 1fr", gap:"32px"}}>{["姓", "名", "電話番号", "シナリオID", "登録日", ...formSettings.map(f=>f.name)].map(f=>(<div key={f} style={{borderBottom:`1px solid ${THEME.border}`, paddingBottom:"12px"}}><label style={{fontSize:"11px", color:THEME.textMuted, fontWeight:"800"}}>{f}</label><div style={{fontWeight:"600", fontSize: "16px", marginTop:"4px"}}>{f==="登録日"||f==="配信予定日時"?formatDate(c[f]):c[f]||"-"}</div></div>))}</div></Page>);
}

// --- 配信状況・表示設定・ユーザー管理 (維持要件) ---
function CustomerSchedule({ customers = [], deliveryLogs = [], onRefresh }) {
  ...
  return (<Page title="配信スケジュール" subtitle={`${c["姓"]}${c["名"]} 様`}><Link to="/" style={{display:"block", marginBottom:"24px", color:THEME.primary, textDecoration:"none", fontWeight:"700"}}>← ダッシュボードへ戻る</Link>
      <div style={{display:"flex", flexDirection:"column", gap:"16px"}}>{myLogs.length > 0 ? myLogs.map((log, i) => (
        ...
        <div style={{ ...s.card, width: "500px", padding: "32px" }}><h3>予定の調整</h3><div style={{marginTop:"20px"}}><label style={{fontSize:"12px"}}>配信日時</label><input type="datetime-local" style={{...s.input, marginTop:"8px"}} value={editLog.t} onChange={e=>setEditLog({...editLog, t:e.target.value})} /></div>
          <div style={{marginTop:"20px"}}><label style={{fontSize:"12px"}}>本文</label><textarea style={{...s.input, height:"150px", marginTop:"8px", resize:"none"}} value={editLog.m} onChange={e=>setEditLog({...editLog, m:e.target.value})} /></div>
          <div style={{display:"flex", gap:"12px", marginTop:"32px"}}><button onClick={async()=>{ await api.post(GAS_URL,{action:"updateDeliveryTime",logId:editLog.id,newTime:editLog.t, newMessage:editLog.m}); onRefresh(); setEditLog(null); }} style={{...s.btn, ...s.btnPrimary, flex:1}}>保存</button><button onClick={()=>setEditLog(null)} style={{...s.btn, ...s.btnSecondary, flex:1}}>閉じる</button></div></div></div>)}</Page>);
}
...
function FormSettings({ formSettings = [], onRefresh }) {
  ...
  return (<Page title="追加項目の調整"><div style={{ maxWidth: "850px" }}>{["姓", "名", "電話番号"].map(f => (<div key={f} style={{ ...s.card, marginBottom: "8px", padding: "16px 24px", display: "flex", gap: "20px", alignItems: "center", backgroundColor: THEME.locked, opacity: 0.7 }}><Lock size={18} color={THEME.textMuted} /><div style={{ flex: 2 }}><label style={{fontSize:"11px"}}>項目名</label><div style={{fontWeight:"700"}}>{f}</div></div><div style={{ flex: 1.5 }}><label style={{fontSize:"11px"}}>形式</label><div>テキスト</div></div></div>))}
    ...
    <button onClick={()=>setItems([...items,{name:"",type:"text",required:true}])} style={{...s.btn, ...s.btnSecondary, width:"100%", borderStyle:"dashed"}}>+ 追加</button><button onClick={async()=>{await api.post(GAS_URL,{action:"saveFormSettings",settings:items});onRefresh();nav("/add")}} style={{...s.btn, ...s.btnPrimary, width:"100%", marginTop:"32px"}}>同期</button></div></Page>);
}
