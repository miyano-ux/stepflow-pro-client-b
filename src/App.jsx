...
import { 
  ...
  Plus, Loader2, LogOut, Users, GripVertical, ListFilter, Edit3, Lock, Save, Search, Clock, ArrowUpDown, ArrowUp, ArrowDown
} from "lucide-react";
...
function Page({ title, children, topButton }) {
  return (<div style={s.main}><div style={{ padding: "40px 60px", maxWidth: "1350px", margin: "0 auto" }}><div style={{ marginBottom: "32px", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}><div><h1 style={{ fontSize: "28px", fontWeight: "800" }}>{title}</h1></div>{topButton}</div>{children}</div></div>);
}

// --- 顧客リスト (並び替え・検索・ソート機能統合版) ---
function CustomerList({ customers, displaySettings, formSettings, onRefresh }) {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (sortConfig.key) {
      result.sort((a, b) => {
        ...
        // 日付・登録日の場合はDateオブジェクトとして比較
        const f = formSettings.find(x => x.name === sortConfig.key);
        if (f?.type === "date" || sortConfig.key === "登録日") {
          const aTime = aVal && aVal !== "-" ? new Date(aVal).getTime() : 0;
          const bTime = bVal && bVal !== "-" ? new Date(bVal).getTime() : 0;
  ...
  return (
    <Page title="顧客ダッシュボード" topButton={<button onClick={() => navigate("/column-settings")} style={{ ...s.btn, ...s.btnSecondary }}><ListFilter size={18} /> 表示・検索設定</button>}>
      {/* 検索バー */}
      ...
      <div style={{ ...s.card, padding: 0, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>
            {visibleCols.map(c => (
              <th key={c} style={{ ...s.tableTh, cursor: "pointer", userSelect: "none" }} onClick={() => requestSort(c)}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  {c}
                  {sortConfig.key === c ? (
                    sortConfig.direction === 'asc' ? <ArrowUp size={14} color={THEME.primary} /> : <ArrowDown size={14} color={THEME.primary} />
          ...
          <tbody>{filteredAndSorted.map(c => (
            <tr key={c.id}>
              {visibleCols.map(col => {
                const f = formSettings.find(x => x.name === col);
                const isDateField = f?.type === "date" || col === "登録日";
                return <td key={col} style={s.tableTd}>{col === "シナリオID" ? <span style={s.badge}>{c[col]}</span> : (isDateField ? formatDate(c[col]) : (c[col] || "-"))}</td>;
              })}
...
function ColumnSettings({ displaySettings, formSettings, onRefresh }) {
  ...
  useEffect(() => {
    ...
    else setItems(["姓", "名", "電話番号", "シナリオID", ...formSettings.map(f => f.name)].map(name => ({ name, visible: true, searchable: true })));
  ...
  const onDragOver = (e, index) => {
    ...
    const newList = [...items]; const draggedItem = newList.splice(dragIdx, 1)[0];
    newList.splice(index, 0, draggedItem); setDragIdx(index); setItems(newList);
  };
  return (<Page title="表示と検索の調整"><div style={{ maxWidth: "700px" }}>
    ...
    <div style={{ marginBottom: "24px" }}>{items.map((it, i) => (<div key={it.name} draggable onDragStart={(e) => onDragStart(e, i)} onDragOver={(e) => onDragOver(e, i)} onDragEnd={() => setDragIdx(null)} style={{ display: "flex", alignItems: "center", gap: "16px", padding: "14px 20px", backgroundColor: "white", border: `1px solid ${dragIdx === i ? THEME.primary : THEME.border}`, borderRadius: "12px", marginBottom: "8px", cursor: "grab", opacity: dragIdx === i ? 0.5 : 1 }}><GripVertical size={18} color={THEME.textMuted} /><div style={{ flex: 1, fontWeight: "600", fontSize: "14px" }}>{it.name}</div><div style={{ width: "80px", display: "flex", justifyContent: "center" }}><input type="checkbox" checked={it.visible} onChange={() => { const n = [...items]; n[i].visible = !n[i].visible; setItems(n); }} style={{ width: "18px", height: "18px" }} /></div><div style={{ width: "80px", display: "flex", justifyContent: "center" }}><input type="checkbox" checked={it.searchable} onChange={() => { const n = [...items]; n[i].searchable = !n[i].searchable; setItems(n); }} style={{ width: "18px", height: "18px" }} /></div></div>))}</div>
...
function CustomerForm({ formSettings, scenarios, onRefresh }) {
  ...
  return (<Page title="新規顧客登録" topButton={<button onClick={() => n("/form-settings")} style={{ ...s.btn, ...s.btnSecondary }}>項目調整</button>}><div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "add", lastName, firstName, phone, data: formData, scenarioID }); onRefresh(); n("/"); }}>
    ...
    <div style={{ borderTop: `1px solid ${THEME.border}`, paddingTop: "24px" }}>{formSettings.filter(f => f.name).map(f => <div key={f.name} style={{marginBottom:"20px"}}><label style={{fontWeight:"700"}}>{f.name}</label><DynamicField f={f} value={formData[f.name]} onChange={val => setFormData({...formData, [f.name]: val})} /></div>)}</div>
...
function ScenarioForm({ scenarios, onRefresh }) {
  ...
  return (<Page title="シナリオ編集"><div style={{ ...s.card, maxWidth: "800px" }}><label>シナリオ名</label><input style={s.input} value={id} onChange={e=>setId(e.target.value)} disabled={!!editId} />
    ...
    <button onClick={async () => { if(!id) return alert("名必須"); setIsSaving(true); try { const cRes = await axios.get(`${GAS_URL}?mode=countAffected&scenarioID=${id}`); if (window.confirm(`${cRes.data.count}名の予約も一括更新されます。？`)) { await api.post(GAS_URL, { action: "saveScenario", scenarioID: id, steps }); alert("完了"); nav("/scenarios"); } } finally { setIsSaving(false); } }} disabled={isSaving} style={{ ...s.btn, ...s.btnPrimary, width: "100%" }}>保存して反映</button></div></Page>);
}
