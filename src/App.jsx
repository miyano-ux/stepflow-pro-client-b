...
import { 
  ...
  Plus, Loader2, LogOut, Users, GripVertical, ListFilter, Edit3, Lock, Search, Clock, Save, ArrowLeft
} from "lucide-react";
...
function CustomerList({ customers, displaySettings, formSettings, onRefresh }) {
  ...
  // 表示列の計算：スプレッドシートの表示設定を優先。設定がなければ基本項目を表示。
  const visibleCols = useMemo(() => {
    if (displaySettings && displaySettings.length > 0) {
      return displaySettings.filter(s => s.visible).map(s => s.name);
    }
  ...
  return (
    <Page title="顧客ダッシュボード" topButton={<button onClick={() => navigate("/column-settings")} style={{ ...s.btn, ...s.btnSecondary }}><ListFilter size={18} /> 表示項目・順序</button>}>
      {/* 検索フィルタバー (表示列と完全に連動) */}
      <div style={{ ...s.card, padding: "16px 20px", marginBottom: "24px", background: "#F1F5F9", border: "none", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button onClick={() => setSearch({})} style={{ fontSize: "12px", color: THEME.primary, background: "none", border: "none", cursor: "pointer", fontWeight: "700", marginLeft: "auto" }}>クリア</button>
      </div>
...
function ColumnSettings({ displaySettings, formSettings, onRefresh }) {
  ...
  useEffect(() => {
    // スプレッドシートの既存設定を読み込む。なければデフォルトを生成。
    if (displaySettings && displaySettings.length > 0) {
      setItems(displaySettings);
    } else {
      const defaults = ["姓", "名", "電話番号", "シナリオID", ...formSettings.map(f => f.name)].map(name => ({ name, visible: true }));
      setItems(defaults);
    }
  ...
  return (
    <Page title="ダッシュボード項目の調整" subtitle="項目の表示・非表示と表示順序をカスタマイズします">
          ...
          {items.map((it, i) => (
            <div key={it.name}
              ...
              style={{ 
                ...
                backgroundColor: "white", border: `1px solid ${dragIdx === i ? THEME.primary : THEME.border}`,
                borderRadius: "12px", marginBottom: "10px", cursor: "grab", transition: "0.1s",
                opacity: dragIdx === i ? 0.5 : 1, boxShadow: dragIdx === i ? "0 8px 16px rgba(0,0,0,0.1)" : "none",
              ...
              <input type="checkbox" checked={it.visible} onChange={() => {
                const n = [...items]; n[i].visible = !n[i].visible; setItems(n);
              }} style={{ width: "20px", height: "20px", cursor: "pointer" }} />
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: "15px" }}>
          <button onClick={handleSave} style={{ ...s.btn, ...s.btnPrimary, flex: 2, padding: "16px" }}><Save size={20}/> サーバー設定を保存</button>
...
// --- 他コンポーネント (ScenarioForm, CustomerForm, etc.) ---
...
function CustomerForm({ formSettings, scenarios, onRefresh }) {
  ...
  return (<Page title="新規顧客登録" topButton={<button onClick={() => n("/form-settings")} style={{ ...s.btn, ...s.btnSecondary }}>追加項目を調整</button>}><div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "add", lastName, firstName, phone, data: formData, scenarioID }); onRefresh(); n("/"); }}>
...
function ScenarioForm({ scenarios, onRefresh }) {
  ...
  return (<Page title="構成"><div style={{ ...s.card, maxWidth: "800px" }}><label>シナリオ名</label><input style={s.input} value={id} onChange={e=>setId(e.target.value)} disabled={!!editId} />
