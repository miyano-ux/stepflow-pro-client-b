...
const s = {
  ...
  card: { backgroundColor: THEME.card, borderRadius: "12px", border: `1px solid ${THEME.border}`, padding: "32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" },
...
function CustomerList({ customers, formSettings, onRefresh }) {
  const [visibleCols, setVisibleCols] = useState(() => JSON.parse(localStorage.getItem("sf_cols_final_v15") || '["姓", "名", "電話番号", "シナリオID"]'));
  const [tempCols, setTempCols] = useState(visibleCols);
  const [showColMenu, setShowColMenu] = useState(false);
  const [search, setSearch] = useState({});
  const [dragIdx, setDragIdx] = useState(null);

  useEffect(() => localStorage.setItem("sf_cols_final_v15", JSON.stringify(visibleCols)), [visibleCols]);
  ...
  return (
    <Page title="顧客ダッシュボード" topButton={<button onClick={() => { setTempCols(visibleCols); setShowColMenu(true); }} style={{ ...s.btn, ...s.btnSecondary }}><ListFilter size={18} /> 表示・順序設定</button>}>
      {showColMenu && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.3)", zIndex: 2000, display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ ...s.card, width: "380px", marginBottom: 0 }}>
            <h3 style={{ marginTop: 0 }}>表示項目の設定</h3>
            <div style={{ maxHeight: "400px", overflowY: "auto", marginBottom: "24px" }}>
              {["姓", "名", "電話番号", "シナリオID", ...formSettings.filter(f => f.name).map(f => f.name)].map((col, i) => (
                <div key={col} draggable onDragStart={(e) => onDragStart(e, i)} onDragOver={(e) => onDragOver(e, i)} onDragEnd={() => setDragIdx(null)}
                  style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px", border: `1px solid ${dragIdx === i ? THEME.primary : THEME.border}`, borderRadius: "8px", marginBottom: "6px", backgroundColor: "white", cursor: "grab", opacity: dragIdx === i ? 0.5 : 1 }}>
      ...
      <div style={{ ...s.card, padding: "16px 20px", marginBottom: "24px", background: "#F1F5F9", border: "none", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        {visibleCols.filter(col => col).map(col => <input key={col} placeholder={`${col}...`} style={{ ...s.input, width: "140px", marginBottom: 0 }} onChange={e => setSearch({...search, [col]: e.target.value})} />)}
      </div>
      <div style={{ ...s.card, padding: 0, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>{visibleCols.filter(c => c).map(c => <th key={c} style={s.tableTh}>{c}</th>)}<th style={s.tableTh}>操作</th></tr></thead>
          <tbody>{filtered.map(c => (
            <tr key={c.id}>
              {visibleCols.filter(col => col).map(col => <td key={col} style={s.tableTd}>{col === "シナリオID" ? <span style={s.badge}>{c[col]}</span> : (c[col] || "-")}</td>)}
