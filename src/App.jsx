        ...
        <tbody>{filteredAndSorted.map(c => (
          <tr key={c.id}>{visibleCols.map(col => <td key={col} style={s.tableTd}>{col === "シナリオID" ? <span style={s.badge}>{c[col]}</span> : (col === "登録日" ? formatDate(c[col]) : (c[col] || "-"))}</td>)}
            <td style={s.tableTd}><div style={{ display: "flex", gap: "15px" }}>
              ...
              <button onClick={async () => { if(window.confirm("この顧客と配信予定も全て削除しますか？")) { await api.post(GAS_URL, { action: "delete", id: c.id }); onRefresh(); } }} style={{ background: "none", border: "none", color: THEME.danger, cursor: "pointer" }}><Trash2 size={16}/></button>
            </div></td></tr>
        ))}</tbody></table></div>
    </Page>
  );
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      const customers = rows.slice(1).filter(r => r.length > 2).map(row => {
        ...
        header.forEach((h, i) => {
          if (h === "姓") obj.lastName = row[i]; else if (h === "名") obj.firstName = row[i]; else if (h === "電話番号") obj.phone = smartNormalizePhone(row[i]); else if (h === "シナリオID") obj.scenarioID = row[i]; else obj.data[h] = row[i];
        }); return obj;
      });
    ...
    <Page title="配信状況" subtitle={`${c["姓"]}${c["名"]} 様`}>
      ...
      <div style={{display:"flex", flexDirection:"column", gap:"16px"}}>{myLogs.length > 0 ? myLogs.map((log, i) => (
        <div key={i} style={{ ...s.card, borderLeft: `6px solid ${log["ステータス"] === "配信済み" ? THEME.success : THEME.primary}`, padding: "20px" }}>
          <div style={{display:"flex", justifyContent:"space-between"}}><div><span style={s.badge}>{log["ステータス"]}</span><div style={{fontWeight:"800", marginTop:"8px"}}>{formatDate(log["配信予定日時"])}</div></div>
          ...
          <div style={{marginTop:"15px", whiteSpace:"pre-wrap", fontSize:"14px"}}>{log["内容"]}</div></div>)) : <div style={{ ...s.card, textAlign: "center", color: THEME.textMuted }}>予定はありません</div>}
      </div>
      {editLog && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}>
        <div style={{ ...s.card, width: "500px" }}><h3>配信の調整</h3><input type="datetime-local" style={s.input} value={editLog.t} onChange={e=>setEditLog({...editLog, t:e.target.value})} /><textarea style={{...s.input, height:"150px", marginTop:"15px", resize:"none"}} value={editLog.m} onChange={e=>setEditLog({...editLog, m:e.target.value})} /><div style={{display:"flex", gap:"12px", marginTop:"20px"}}><button onClick={async()=>{await api.post(GAS_URL,{action:"updateDeliveryTime",logId:editLog.id,newTime:editLog.t, newMessage:editLog.m}); onRefresh(); setEditLog(null);}} style={{...s.btn, ...s.btnPrimary, flex:1}}>保存</button><button onClick={()=>setEditLog(null)} style={{...s.btn, ...s.btnSecondary, flex:1}}>閉じる</button></div></div></div>)}</Page>);
}
...
function CustomerDetail({ customers = [], formSettings = [] }) {
  ...
  return (<Page title="詳細"><Link to="/" style={{display:"block", marginBottom:"24px", color:THEME.primary, textDecoration:"none", fontWeight:"700"}}>← 戻る</Link>
