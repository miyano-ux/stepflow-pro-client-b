...
function CustomerList({ customers, formSettings, onRefresh }) {
  const [visibleCols, setVisibleCols] = useState(() => JSON.parse(localStorage.getItem("sf_cols_final_v10") || '["姓", "名", "電話番号", "シナリオID"]'));
  const [tempCols, setTempCols] = useState(visibleCols);
  const [showColMenu, setShowColMenu] = useState(false);
  const [search, setSearch] = useState({});
  const [dragIdx, setDragIdx] = useState(null);

  useEffect(() => localStorage.setItem("sf_cols_final_v10", JSON.stringify(visibleCols)), [visibleCols]);
  ...
  const handleDragEnter = (idx) => {
    ...
    const draggedItem = newList.splice(dragIdx, 1)[0];
  ...
  return (
    <Page title="顧客ダッシュボード" topButton={<button onClick={() => { setTempCols(visibleCols); setShowColMenu(true); }} style={{ ...s.btn, backgroundColor: "white", color: THEME.textMain, border: `1px solid ${THEME.border}` }}><ListFilter size={18} /> 表示・順序設定</button>}>
      {showColMenu && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.3)", zIndex: 2000, display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ ...s.card, width: "380px", marginBottom: 0 }}>
            <h3 style={{ marginTop: 0, marginBottom: "8px" }}>表示項目の設定</h3>
            <p style={{ fontSize: "12px", color: THEME.textMuted, marginBottom: "20px" }}>ドラッグで並び替えが可能です</p>
            <div style={{ maxHeight: "400px", overflowY: "auto", marginBottom: "24px" }}>
              {["姓", "名", "電話番号", "シナリオID", ...formSettings.map(f => f.name)].map((col, i) => (
                <div key={col} draggable onDragStart={() => handleDragStart(i)} onDragEnter={() => handleDragEnter(i)} onDragOver={e => e.preventDefault()}
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button onClick={() => { setVisibleCols(tempCols); setShowColMenu(false); }} style={{ ...s.btn, backgroundColor: THEME.primary, color: "white", flex: 1 }}>適用</button>
              <button onClick={() => setShowColMenu(false)} style={{ ...s.btn, backgroundColor: "white", border: `1px solid ${THEME.border}`, flex: 1 }}>キャンセル</button>
            </div>
          </div>
        </div>
      )}
      <div style={{ ...s.card, padding: "16px 20px", marginBottom: "24px", background: "#F1F5F9", border: "none", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        {visibleCols.map(col => {
          ...
          if (f?.type === "dropdown") return <select key={col} style={{ ...s.input, width: "160px" }} onChange={e => setSearch({...search, [col]: e.target.value})}><option value="">{col}: 全て</option>{f.options?.split(",").map(o => <option key={o} value={o}>{o}</option>)}</select>;
          return <input key={col} placeholder={`${col}...`} style={{ ...s.input, width: "140px" }} onChange={e => setSearch({...search, [col]: e.target.value})} />;
        })}
      </div>
      <div style={{ ...s.card, padding: 0, overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{visibleCols.map(c => <th key={c} style={s.tableTh}>{c}</th>)}<th style={s.tableTh}>操作</th></tr></thead><tbody>{filtered.map(c => (
        <tr key={c.id}><td key="cols" style={{display:"none"}}>{visibleCols.map(col => <td key={col} style={s.tableTd}>{col === "シナリオID" ? <span style={s.badge}>{c[col]}</span> : (c[col] || "-")}</td>)}</td>
          {visibleCols.map(col => <td key={col} style={s.tableTd}>{col === "シナリオID" ? <span style={s.badge}>{c[col]}</span> : (c[col] || "-")}</td>)}
          <td style={s.tableTd}><div style={{ display: "flex", gap: "15px" }}><Link to={`/schedule/${c.id}`} style={{ textDecoration: "none", color: THEME.primary, fontWeight: "700", display: "flex", alignItems: "center", gap: "4px" }}><Clock size={16}/> 状況</Link><Link to={`/edit/${c.id}`} style={{ textDecoration: "none", color: THEME.textMuted, display: "flex", alignItems: "center", gap: "4px" }}><Edit3 size={16}/> 編集</Link><button onClick={async () => { if(window.confirm("削除？")) { await api.post(GAS_URL, { action: "delete", id: c.id }); onRefresh(); } }} style={{ background: "none", border: "none", color: THEME.danger, cursor: "pointer" }}><Trash2 size={16}/></button></div></td>
        </tr>
      ))}</tbody></table></div>
    </Page>
  );
...
function CustomerForm({ formSettings, scenarios, onRefresh }) {
  ...
  return (
    <Page title="新規顧客登録" topButton={<button onClick={() => navigate("/form-settings")} style={{ ...s.btn, backgroundColor: "white", border: `1px solid ${THEME.border}`, color: THEME.textMain }}>項目を調整</button>}>
      <div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "add", lastName, firstName, phone, data: formData, scenarioID }); onRefresh(); navigate("/"); }}>
        ...
        <button type="submit" style={{ ...s.btn, backgroundColor: THEME.primary, color: "white", width: "100%", padding: "14px" }}>登録する</button>
      </form></div>
    </Page>
  );
}
  ...
  return (
    <Page title="情報の編集">
      <div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "update", id, lastName, firstName, phone, data: formData, status: c.配信ステータス, scenarioID }); onRefresh(); nav("/"); }}>
        ...
        <button type="submit" style={{ ...s.btn, backgroundColor: THEME.primary, color: "white", width: "100%", padding: "14px" }}>保存</button>
      </form></div>
    </Page>
  );
}
...
function UserManager({ masterUrl }) {
  ...
  return (
    <Page title="ユーザー管理" topButton={<button onClick={() => setModal({ open: true, mode: "add", data: { name: "", email: "" } })} style={{ ...s.btn, backgroundColor: THEME.primary, color: "white" }}><Plus size={18} /> 追加</button>}>
      <div style={{ ...s.card, padding: 0 }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th style={s.tableTh}>名前</th><th style={s.tableTh}>メール</th><th style={{...s.tableTh, textAlign:"right"}}>操作</th></tr></thead><tbody>{users.map((u, i) => (
        <tr key={i}><td style={s.tableTd}>{u.name}</td><td style={s.tableTd}>{u.email}</td><td style={{...s.tableTd, textAlign:"right"}}><div style={{display:"flex", gap:"10px", justifyContent:"flex-end"}}>
          ...
          <button onClick={async()=>{if(window.confirm("削除？")){await api.post(masterUrl,{action:"deleteUser",company:CLIENT_COMPANY_NAME,email:u.email});fetchUsers();}}} style={{background:"none", border:"none", color:THEME.danger, cursor:"pointer"}}><Trash2 size={16}/></button>
        </div></td></tr>
      ))}</tbody></table></div>
      {modal.open && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}>
          <div style={{ ...s.card, width: "400px", marginBottom: 0 }}><h3>{modal.mode === "add" ? "追加" : "編集"}</h3>
            ...
            <div style={{ display: "flex", gap: "10px" }}><button type="submit" style={{...s.btn, backgroundColor:THEME.primary, color:"white", flex:1}}>保存</button><button type="button" onClick={()=>setModal({open:false})} style={{...s.btn, backgroundColor:"white", border:`1px solid ${THEME.border}`, flex:1}}>閉じる</button></div></form>
          </div>
        </div>
      )}
    </Page>
...
function ScenarioList({ scenarios, onRefresh }) {
  ...
  return (<Page title="シナリオ管理" topButton={<Link to="/scenarios/new" style={{...s.btn, backgroundColor: THEME.primary, color:"white", textDecoration:"none"}}><Plus size={18}/> 新規</Link>}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "24px" }}>{Object.entries(grouped).map(([id, steps]) => (<div key={id} style={s.card}><div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"20px"}}><div><h3 style={{margin:0, fontSize:"18px"}}>{id}</h3><span style={{fontSize:"13px", color:THEME.textMuted}}>{steps.length} ステップ</span></div><button onClick={()=>del(id)} style={{color:THEME.danger, background:"none", border:"none", cursor:"pointer"}}><Trash2 size={18}/></button></div><Link to={`/scenarios/edit/${id}`} style={{ ...s.btn, backgroundColor: "white", border: `1px solid ${THEME.border}`, color: THEME.textMain, width: "100%", textDecoration: "none" }}>構成を編集</Link></div>))}</div></Page>);
}
function ScenarioForm({ scenarios, onRefresh }) {
  ...
  return (<Page title="シナリオ編集"><div style={{ ...s.card, maxWidth: "800px" }}><label>シナリオ名</label><input style={s.input} value={id} onChange={e=>setId(e.target.value)} disabled={!!editId} />
    ...
    <button onClick={() => setSteps([...steps, { elapsedDays: 1, deliveryHour: 10, message: "" }])} style={{ ...s.btn, backgroundColor: "white", border: `1px solid ${THEME.border}`, width: "100%", marginBottom: "15px" }}>+ ステップ追加</button>
    <button onClick={async () => { if(!id) return alert("名必須"); setIsSaving(true); try { const cRes = await axios.get(`${GAS_URL}?mode=countAffected&scenarioID=${id}`); if (window.confirm(`${cRes.data.count}名の予約も一括更新されます。よろしいですか？`)) { await api.post(GAS_URL, { action: "saveScenario", scenarioID: id, steps }); alert("更新完了"); nav("/scenarios"); } } finally { setIsSaving(false); } }} disabled={isSaving} style={{ ...s.btn, backgroundColor: THEME.primary, color: "white", width: "100%" }}>保存して反映</button></div></Page>);
}
function CustomerSchedule({ customers, deliveryLogs, onRefresh }) {
  ...
  return (<Page title="配信状況" subtitle={`${c["姓"]}${c["名"]} 様`}><Link to="/" style={{display:"block", marginBottom:"20px", color:THEME.primary, textDecoration:"none", fontWeight:"700"}}>← 戻る</Link><div style={{display:"flex", flexDirection:"column", gap:"16px"}}>{myLogs.length > 0 ? myLogs.map((log, i) => (<div key={i} style={{ ...s.card, borderLeft: `6px solid ${log.ステータス === "配信済み" ? THEME.success : THEME.primary}`, marginBottom: 0 }}><div style={{display:"flex", justifyContent:"space-between"}}><div><span style={s.badge}>{log.ステータス}</span><div style={{fontWeight:"800", marginTop:"8px"}}>{new Date(log.配信予定日時).toLocaleString()}</div></div>{log.ステータス === "配信待ち" && <button onClick={()=>setEditLog({...log, t: new Date(log.配信予定日時).toISOString().slice(0,16), m: log.内容})} style={{color:THEME.primary, background:"none", border:"none", cursor:"pointer", fontWeight:"600"}}>編集</button>}</div><div style={{marginTop:"12px", whiteSpace:"pre-wrap", fontSize:"14px"}}>{log.内容}</div></div>)) : <div style={s.card}>データがありません。</div>}</div>
  {editLog && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}><div style={{ ...s.card, width: "500px", marginBottom: 0 }}><h3>配信内容の個別調整</h3><div style={{marginBottom:"15px"}}><label style={{fontSize:"12px"}}>配信日時</label><input type="datetime-local" style={s.input} value={editLog.t} onChange={e=>setEditLog({...editLog, t:e.target.value})} /></div><div style={{marginBottom:"20px"}}><label style={{fontSize:"12px"}}>本文</label><textarea style={{...s.input, height:"150px"}} value={editLog.m} onChange={e=>setEditLog({...editLog, m:e.target.value})} /></div><div style={{display:"flex", gap:"12px"}}><button onClick={async()=>{await api.post(GAS_URL,{action:"updateDeliveryTime",logId:editLog.ログID,newTime:editLog.t, newMessage:editLog.m}); onRefresh(); setEditLog(null);}} style={{...s.btn, backgroundColor:THEME.primary, color:"white", flex:1}}>保存</button><button onClick={()=>setEditLog(null)} style={{...s.btn, backgroundColor:"white", border:`1px solid ${THEME.border}`, flex:1}}>閉じる</button></div></div></div>)}</Page>);
}
function FormSettings({ formSettings, onRefresh }) {
  ...
  <button onClick={() => setItems([...items, { name: "", type: "text", required: true, options: "" }])} style={{ ...s.btn, backgroundColor: "white", border: `2px dashed ${THEME.border}`, width: "100%" }}>+ 追加</button><button onClick={async () => { await api.post(GAS_URL, { action: "saveFormSettings", settings: items }); onRefresh(); nav("/add"); }} style={{ ...s.btn, backgroundColor: THEME.primary, color: "white", width: "100%", marginTop: "32px", padding:"14px" }}>同期する</button></div></Page>);
}
