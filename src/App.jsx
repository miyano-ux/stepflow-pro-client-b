...
function CustomerList({ customers, formSettings, onRefresh }) {
  const [visibleCols, setVisibleCols] = useState(() => JSON.parse(localStorage.getItem("sf_cols_final_v8") || '["姓", "名", "電話番号", "シナリオID"]'));
  const [tempCols, setTempCols] = useState(visibleCols);
  const [showColMenu, setShowColMenu] = useState(false);
  const [search, setSearch] = useState({});

  useEffect(() => localStorage.setItem("sf_cols_final_v8", JSON.stringify(visibleCols)), [visibleCols]);
  ...
  return (
    <Page title="顧客ダッシュボード" topButton={<button onClick={() => setShowColMenu(true)} style={{ ...s.btn, backgroundColor: "white", color: THEME.textMain, border: `1px solid ${THEME.border}` }}><ListFilter size={18} /> 表示項目</button>}>
      {showColMenu && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.2)", zIndex: 1000, display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ ...s.card, width: "320px", marginBottom: 0 }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button onClick={() => { setTempCols(visibleCols); setShowColMenu(false); }} style={{ ...s.btn, backgroundColor: "white", color: THEME.textMain, border: `1px solid ${THEME.border}`, flex: 1 }}>キャンセル</button>
            </div>
          </div>
        </div>
      )}
      <div style={{ ...s.card, padding: "16px 20px", marginBottom: "24px", background: "#F1F5F9", border: "none", display: "flex", gap: "12px", alignItems: "center" }}>
...
function CustomerForm({ formSettings, scenarios, onRefresh }) {
  ...
  return (
    <Page title="新規顧客登録" topButton={<button onClick={() => n("/form-settings")} style={{ ...s.btn, backgroundColor: "white", border: `1px solid ${THEME.border}`, color: THEME.textMain }}>項目を調整</button>}>
      <div style={{ ...s.card, maxWidth: "650px", margin: "0 auto" }}><form onSubmit={async (e) => { e.preventDefault(); await api.post(GAS_URL, { action: "add", lastName, firstName, phone, data: formData, scenarioID }); onRefresh(); n("/"); }}>
...
// --- 他コンポーネント (ScenarioForm, Schedule, FormSettings, Detail) ---
function ScenarioForm({ scenarios, onRefresh }) {
  ...
  return (<Page title="シナリオ編集"><div style={{ ...s.card, maxWidth: "800px" }}><label style={{fontWeight:"700", display:"block", marginBottom:"8px"}}>シナリオ名</label><input style={s.input} value={id} onChange={e=>setId(e.target.value)} disabled={!!editId} />
    ...
    <button onClick={() => setSteps([...steps, { elapsedDays: 1, deliveryHour: 10, message: "" }])} style={{ ...s.btn, backgroundColor: "white", border: `1px solid ${THEME.border}`, width: "100%", marginBottom: "15px" }}>+ 追加</button>
...
function CustomerSchedule({ customers, deliveryLogs, onRefresh }) {
  ...
  return (<Page title="配信状況" subtitle={`${c["姓"]}${c["名"]} 様`}><Link to="/" style={{display:"block", marginBottom:"20px", color:THEME.primary, textDecoration:"none", fontWeight:"700"}}>← 戻る</Link><div style={{display:"flex", flexDirection:"column", gap:"16px"}}>{myLogs.map((log, i) => (<div key={i} style={{ ...s.card, borderLeft: `6px solid ${log.ステータス === "配信済み" ? THEME.success : THEME.primary}`, marginBottom: 0 }}><div style={{display:"flex", justifyContent:"space-between"}}><div><span style={s.badge}>{log.ステータス}</span><div style={{fontWeight:"800", marginTop:"8px"}}>{new Date(log.配信予定日時).toLocaleString()}</div></div>{log.ステータス === "配信待ち" && <button onClick={()=>setEditLog(log)} style={{color:THEME.primary, background:"none", border:"none", cursor:"pointer", fontWeight:"600"}}>日時変更</button>}</div><div style={{marginTop:"12px", whiteSpace:"pre-wrap", fontSize:"14px"}}>{log.内容}</div></div>))}</div>
  {editLog && (<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}><div style={{ ...s.card, width: "400px", marginBottom: 0 }}><h3>日時変更</h3><input type="datetime-local" style={s.input} onChange={e=>setEditLog({...editLog, t:e.target.value})} /><div style={{display:"flex", gap:"10px", marginTop:"20px"}}><button onClick={async()=>{await api.post(GAS_URL,{action:"updateDeliveryTime",logId:editLog.ログID,newTime:editLog.t}); onRefresh(); setEditLog(null);}} style={{...s.btn, backgroundColor:THEME.primary, color:"white", flex:1}}>保存</button><button onClick={()=>setEditLog(null)} style={{...s.btn, backgroundColor:"white", border:`1px solid ${THEME.border}`, flex:1}}>閉じる</button></div></div></div>)}</Page>);
}
