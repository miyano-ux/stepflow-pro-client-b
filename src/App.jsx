...
function CustomerList({ customers = [], displaySettings = [], formSettings = [], onRefresh }) {
  ...
  return (
    <Page title="顧客ダッシュボード" topButton={<div style={{ display: "flex", gap: "10px" }}>
...
function DirectSms({ customers = [], templates = [], onRefresh }) {
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    try {
      await api.post(GAS_URL, { action: "sendDirectSms", phone: c["電話番号"], customerName: `${c["姓"]} ${c["名"]}`, scheduledTime: time, message: msg });
      alert("配信予約完了"); navigate("/");
    } catch (e) { alert(e.message); } finally { setLoading(false); }
  };
  return (
    <Page title="個別SMS配信" subtitle={`${c["姓"]} ${c["名"]} 様へのメッセージ`}>
      <Link to="/" style={{ display: "block", marginBottom: "24px", color: THEME.primary, textDecoration: "none", fontWeight: "700" }}>← 戻る</Link>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 350px", gap: "32px" }}>
        <div style={s.card}>
          <label style={{ fontWeight: "700", display: "block", marginBottom: "8px" }}>予定日時</label>
          <input type="datetime-local" style={{...s.input, marginBottom:"20px"}} value={time} onChange={e => setTime(e.target.value)} />
          <label style={{ fontWeight: "700", display: "block", marginBottom: "8px" }}>本文</label>
          <textarea style={{ ...s.input, height: "250px", resize: "none" }} value={msg} onChange={e => setMsg(e.target.value)} />
          <button onClick={handleSend} disabled={loading} style={{ ...s.btn, ...s.btnPrimary, width: "100%", marginTop: "24px" }}>
            {loading ? <Loader2 className="animate-spin" /> : <><Send size={18}/> 配信予約を確定</>}
          </button>
        </div>
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {templates.map(t => (
              <div key={t.id} onClick={() => setMsg(replaceVariables(t.content, c))} style={{ ...s.card, padding: "16px", cursor: "pointer", border: `1px solid ${THEME.border}` }}>
                <div style={{ fontWeight: "700", fontSize: "14px" }}>{t.name}</div>
                <div style={{ fontSize: "12px", color: THEME.textMuted, marginTop:"4px" }}>{t.content.slice(0, 50)}...</div>
              </div>
            ))}
          </div>
        </div>
...
function TemplateManager({ templates = [], onRefresh }) {
  ...
  const sub = async (e) => {
    ...
    alert("保存完了"); setModal({ open: false, data: { id: "", name: "", content: "" } }); onRefresh();
  };
  return (
          ...
          <div key={t.id} style={s.card}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button onClick={() => setModal({ open: true, data: t })} style={{ background: "none", border: "none", color: THEME.textMuted }}><Edit3 size={16}/></button>
                <button onClick={async() => { if(window.confirm("削除？")){ await api.post(GAS_URL, { action: "deleteTemplate", id: t.id }); onRefresh(); }}} style={{ background: "none", border: "none", color: THEME.danger }}><Trash2 size={16}/></button>
              </div>
            </div>
            <pre style={{ fontSize: "13px", color: THEME.textMuted, whiteSpace: "pre-wrap", background: "#F8FAFC", padding: "12px", borderRadius: "8px" }}>{t.content}</pre>
          </div>
        ))}
      </div>
      {modal.open && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}>
          <div style={{ ...s.card, width: "600px" }}>
            <h3>テンプレート編集</h3>
            <form onSubmit={sub}>
              <input style={{ ...s.input, marginBottom: "16px" }} value={modal.data.name} onChange={e => setModal({...modal, data: {...modal.data, name: e.target.value}})} placeholder="テンプレート名" required />
              <textarea style={{ ...s.input, height: "200px", resize: "none", marginBottom: "20px" }} value={modal.data.content} onChange={e => setModal({...modal, data: {...modal.data, content: e.target.value}})} placeholder="本文 ({{姓}} 様 など)" required />
...
function CustomerForm({ formSettings = [], scenarios = [], onRefresh }) {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      try { await api.post(GAS_URL, { action: "bulkAdd", customers }); alert("登録完了"); onRefresh(); navigate("/"); } catch (err) { alert(err.message); }
...
function CustomerSchedule({ customers = [], deliveryLogs = [], onRefresh }) {
  ...
  return (<Page title="配信状況" subtitle={`${c["姓"]}${c["名"]} 様`}>
      <Link to="/" style={{display:"block", marginBottom:"24px", color:THEME.primary, textDecoration:"none", fontWeight:"700"}}>← 戻る</Link>
